...
class StudentSolver(Node):
    ...
    def scan_callback(self, msg):
        ...
        #-------- DEMO LOGIC, REMOVE THIS AND WRITE YOUR OWN ---------
       	if d_front < 0.4: # if shit ahead turn right

            cmd.angular.z = -67.0 # wanted to max out so entered a vaguely big number, common theme
            
        elif d_left < 0.6: # if shit to your left go straight but like, a little right too so you don't run straight into corners

            cmd.linear.x = 100.0
            
            cmd.angular.z = -1.0

        elif d_left > 1.0: # sails to port when nun to your left

            cmd.angular.z = 200.0

            cmd.linear.x = 0.7
            
        else: # nun complicated, just go straight and a lil left, corrected by second elif
